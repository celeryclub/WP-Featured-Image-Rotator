...
$start_cimy_hir_comment .= "\tCopyright (c) 2009-2011 Marco Cimmino\n";
...
function hir_add_css() {
	global $cimy_hir_curr_options, $start_cimy_hir_comment, $end_cimy_hir_comment;
   ...
   if (myid[0]) {
    $('#$div_id').crossSlide({\n";
		...
		while ($i != $tot_images) {
			...
			echo "\n\t{ src: '".esc_js(IMAGE_URI.$id.'/'.$filename)."'";
...
function cimy_hir_manage_upload($input_name, $rules, $id) {
	...
	if ((!isset($_FILES[$input_name]['name'])) || (!is_writable(IMAGE_PATH.$id)))
		return "";
	
	// picture filesystem path
	$file_full_path = IMAGE_PATH.$id."/".$file_name;
...
function rotator_options() {
	...
	if (isset($_POST["upload"])) {
		$upload_res = cimy_hir_manage_upload("userfile", false, $id);
	}
